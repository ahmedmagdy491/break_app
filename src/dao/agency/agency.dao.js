...
class AgencyDAO {
  ...
  static approveAgencyJoinReqs(agencyId, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!(await checkUpdated(agency))) {
          reject(
            new Error("your request not completed, please try again later.")
          );
        }
        resolve({ success: true });
      } catch (error) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
                    ...
                    $cond: [
                      { $eq: ["$$product.id", giftId] },
                      {
                        $mergeObjects: [
                          ...
                          {
                            quantity: {
                              $subtract: ["$$product.quantity", giftQty],
                            },
                          },
                        ],
                      },
        ...
        await Transfers.create({
          from: userId,
          to: memberId,
          quantity: giftQty * gift_price.price * beans_golds_eq.beans_golds,
        });
        resolve({ success: true });
      } catch (error) {
        reject(error);
  ...
  static awardAgencyMembers(agencyId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          agency.total_balance.current_value < agency.total_balance.target_value
        ) {
          reject(
            new Error("The current balalnce is less than The target balance")
          );
        }
            ...
            $set: {
              "wallet.beans": {
                $add: [
                  "$wallet.beans",
                  {
                    $multiply: ["$total_balance.current_value", 0.1],
                  },
                ],
              },
                ...
                $add: [
                  "$wallet.beans",
                  {
                    $subtract: [
                      "$agency.total_balance",
                      {
                        $multiply: ["$agency.total_balance", 0.1],
                      },
                    ],
                  },
                ],
  ...
  static updateAgency(info) {
    return new Promise(async (resolve, reject) => {
      try {
        const updateAgency = await Agency.updateOne(
          { _id: info.id },
          { ...info }
        );
        resolve(updateAgency.modifiedCount);
      } catch (error) {
        reject(error);
      }
    });
  ...
  static deleteAgency(id) {
    return new Promise(async (resolve, reject) => {
      try {
        const deleteAgency = await Agency.deleteOne({ _id: id });
        resolve(deleteAgency.modifiedCount);
      } catch (error) {
        reject(error);
      }
    });
